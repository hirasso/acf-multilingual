...
class ACFMultilingual
{
    ...
    public function add_multilingual_object_types()
    {
        ...
        if ($this->config->taxonomies) {
            $this->taxonomies_controller->add_taxonomies($this->config->taxonomies);
        }
    }
    ...
    public function home_url(string $path = '', ?string $lang = null): string
    {
        ...
        return \trailingslashit($home_url) . $lang . $path;
    }
    ...
    public function get_template($template_name, $value = null, $allow_filter = true): string
    {
        ...
        if ($allow_filter) {
            $path = \apply_filters("acfml/template/$template_name", $path);
        }
        if (!\file_exists($path)) {
            return "<p>$template_name: Template doesn't exist</p>";
        }
        \ob_start();
        if ($this->is_dev()) {
    ...
    public function get_language_switcher(?array $args = [])
    {
        ...
        foreach ($languages as $key => &$language) {
            ...
            if ($language['is_current']) {
                $language['html_classes'][] = 'is-current-language';
            }
            if ($this->is_default_language($language['slug'])) {
                $language['html_classes'][] = 'is-default-language';
            }
            if ($args->hide_current && $language['is_current']) {
                unset($languages[$key]);
            }
            $this->debug = true;
    ...
    public function detect_language(): void
    {
        ...
        if (!\defined('ACFML_CURRENT_LANGUAGE')) {
    ...
    */
    public function convert_url(?string $url = null, ?string $requested_language = null): string
    {
        ...
        if (!$requested_language) {
            $requested_language = $this->get_current_language();
        }
        ...
        if ($this->url_starts_with($url, \content_url())) {
            return $url;
        }
        ...
        if ($this->get_language_in_url($url) === $requested_language) {
            return $url;
        }
    ...
    {
        if (!$this->is_default_language($this->get_language_in_url($url))) {
            return $url;
        }
    ...
    */
    ...
    private function is_internal_url(string $url): bool
    {
        ...
        if (!$this->url_starts_with($url, (string) \home_url())) {
            return false;
        }

        if ($this->url_points_to_physical_location($url)) {
            return false;
        }
    ...
    {
        if (!$this->current_language_is_default()) {
            return;
        }
